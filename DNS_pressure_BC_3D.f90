...
!============================================================================
    subroutine initialize_fields_3d()
        ...
        ! OPTIMIZED: Cache-friendly loop order and cached calculations
        real(wp) :: ylen_over_ny, xlen_over_nx
        ylen_over_ny = ylen / real(ny, wp)
        xlen_over_nx = xlen / real(nx, wp)
        
        do i = 1, nx
            x = real(i-1, wp) * xlen_over_nx
            do j = 1, ny
                y = real(j-1, wp) * ylen_over_ny
                do k = 1, nz
                    z = zpts(k)
...
!   Where integration uses:
!   - Periodic spectral integration in x and y directions (simple averaging)
!   - LGL quadrature weights in z direction for proper integration over [-1,1]
!
! MATHEMATICAL FORMULATION:
!   For periodic x,y: ∫∫ u dx dy = (Lx*Ly/(nx*ny)) * Σᵢⱼ u(i,j,k)
!   For z: ∫ u dz = (ybar/2) * Σₖ u(i,j,k) * zwts(k)  [ybar/2 maps [-1,1] to physical]
!   Total: U_bulk = (1/(Lx*Ly*ybar)) * ΣᵢⱼₖK u(i,j,k) * (Lx/nx) * (Ly/ny) * (ybar/2) * zwts(k)
!          Simplifies to: U_bulk = (1/(nx*ny*2)) * ΣᵢⱼₖK u(i,j,k) * zwts(k)
!============================================================================
    subroutine calculate_bulk_velocity_3d(u_bulk)
        implicit none
        real(wp), intent(out) :: u_bulk
        real(wp) :: volume_integral, weight_sum
        integer :: i, j, k
        ...
        ! For 3D channel flow: integrate u(x,y,z) over entire volume
        do k = 1, nz
            do j = 1, ny
                do i = 1, nx
                    ! Volume integration with proper weights:
                    ! - x,y: periodic (uniform spacing) - simple sum
                    ! - z: LGL quadrature weights for [-1,1] domain
                    volume_integral = volume_integral + u(i,j,k) * zwts(k)
                end do
            end do
            ! Add weight only once per k-level, not once per (i,j,k) point!
            weight_sum = weight_sum + zwts(k)
        end do
        
        ! Complete the integration:
        ! - Normalize by number of x,y points for periodic average
        ! - weight_sum normalizes the z-integration (LGL weights sum to 2 for [-1,1])
        u_bulk = volume_integral / (real(nx*ny, wp) * weight_sum)
