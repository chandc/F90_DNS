! =============================================================================
! 3D NAVIER-STOKES CHANNEL FLOW SOLVER - CRANK-NICOLSON DEVELOPMENT
! =============================================================================
! Originally developed by Daniel Chiu-Leung Chan, 1993
! Modernized conversion from base_code.f to F90 format
! 
! CRANK-NICOLSON IMPLEMENTATION IN PROGRESS
! 
! Status: DEVELOPMENT VERSION - IMPLEMENTING CN VISCOUS STEP
! - Base: Working backward Euler (see backup: base_code_backward_euler_working.f90)
...
contains
    ...
    subroutine solve_pressure_step(p)
        ...
        allocate(poisson_matrix(nz, nz), div_vec(nz), phi_vec(nz), ipiv(nz))
        ...
        do ix = 1, nxhp
            ...
            if (ix == 1) then
                ! For kx=0 mode, regularize the system to remove null space
                ! Use a modified Poisson equation with integral constraint
                
                ! Build Poisson matrix: D²/dy²
                poisson_matrix = 0.0_wp
                do i = 1, nz
                    do j = 1, nz
                        if (i <= nzm .and. j <= nzm) then
                            poisson_matrix(i,j) = compute_second_derivative_element(p, i-1, j-1)
                        endif
                    end do
                end do
                
                ! Apply boundary conditions
                ! Bottom: φ = 0 (Dirichlet)
                poisson_matrix(1,:) = 0.0_wp
                poisson_matrix(1,1) = 1.0_wp
                
                ! Top: φ = 0 (Dirichlet)  
                poisson_matrix(nz,:) = 0.0_wp
                poisson_matrix(nz,nz) = 1.0_wp
                
                ! Regularize: modify one interior equation to fix integral of φ
                ! Replace middle equation with: φ(middle) = 0
                if (nz > 2) then
                    poisson_matrix(nz/2,:) = 0.0_wp
                    poisson_matrix(nz/2,nz/2) = 1.0_wp
                endif
                
                ! Right-hand side: use divergence without extra scaling
                div_vec = div_spectral(ix,:)
                div_vec(1) = 0.0_wp   ! φ = 0 at bottom
                div_vec(nz) = 0.0_wp  ! φ = 0 at top
                if (nz > 2) then
                    div_vec(nz/2) = 0.0_wp  ! φ = 0 at middle (regularization)
                endif
                
            else
                ! For kx ≠ 0 modes, build standard Poisson matrix: D²/dy² - λₓ² I
                poisson_matrix = 0.0_wp
                ...
                do i = 1, nz
                    ...
                    if (i <= nzm) then
                        poisson_matrix(i,i) = poisson_matrix(i,i) - lambda_x
                    endif
                end do
                
                ! Apply Dirichlet boundary conditions: φ = 0 at walls
                ! This removes singularity for non-zero modes
                poisson_matrix(1,:) = 0.0_wp
                poisson_matrix(1,1) = 1.0_wp
            ...
            call dgesv(nz, 1, poisson_matrix, nz, ipiv, div_vec, nz, info)
            if (info /= 0) then
                write(*,*) 'Error in pressure Poisson solve, info =', info, 'for mode ix =', ix
                ! Set to zero if singular
                phi_spectral(ix,:) = 0.0_wp
            else
                phi_spectral(ix,:) = div_vec
            endif
        ...
        deallocate(poisson_matrix, div_vec, phi_vec, ipiv)
    ...
    subroutine project_velocity_to_divergence_free(p)
        ...
        do ix = 1, nxhp
            ...
            ! Solve linear system
            phi_vec = div_vec
            call dgesv(nz, 1, poisson_matrix, nz, ipiv, phi_vec, nz, info)
            
            if (info /= 0) then
                write(*,*) 'Error in initial projection Poisson solve, info =', info, 'for mode ix =', ix
                phi_spectral(ix,:) = 0.0_wp
            else
